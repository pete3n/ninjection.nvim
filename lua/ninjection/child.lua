...
function NJChild:init_buf(opts)
  ...
  if opts.create_win then
    ...
    if not c_win or cwin_err then
      ---@type string
      local err = 'ninjection.child:init_buf() error: Failed to create child window... '
        .. tostring(cwin_err)
      if cfg.debug then
        vim.notify(err, vim.log.levels.ERROR)
      end
  ...
  ---@type boolean
  local sname_ok =
    pcall(vim.api.nvim_buf_set_name, c_bufnr, self.p_name .. ':' .. c_bufnr .. ':' .. self.c_ft)
...
function NJChild:set_nj_table()
  ...
  if not set_nj_ok then
    ---@type string
    local err =
      'ninjection.child:set_nj_table() error: Failed to update ninjection table with child object.'
    if cfg.debug then
      vim.notify(err, vim.log.levels.ERROR)
    end
    return false, err
