...
function ninjection.select()
  ...
  if not injection or not injection.pair.node then
    if cfg.debug then
      vim.notify(
        'ninjection.select() warning: No valid TSNode returned: ' .. tostring(err),
        vim.log.levels.WARN
      )
    end
    return nil
  ...
  if not v_range then
    if cfg.debug then
      vim.notify(
        'ninjection.select() warning: no visual range returned: ' .. tostring(err),
        vim.log.levels.WARN
      )
    end
    return nil
...
function ninjection.edit()
  ...
  if not root_dir then
    ---@type string
    local err = 'ninjection.edit() error: Failed to get current working directory ... '
      .. tostring(dir_err)
    if cfg.debug then
      vim.notify(err, vim.log.levels.ERROR)
    end
  ...
  if not init_ok then
    ---@type string
    local err = 'ninjection.edit() error: Could not initialize Ninjection child ... '
      .. tostring(init_err)
    if cfg.debug then
      vim.notify(err, vim.log.levels.ERROR)
    end
  ...
  if not reg_ok then
    ---@type string
    local err = 'ninjection.edit() error: Failed to register child buffer with parent...'
      .. tostring(reg_err)
    if cfg.debug then
      vim.notify(err, vim.log.levels.ERROR)
    end
...
function ninjection.replace()
  ...
  if cfg.debug then
    ...
    elseif not cfg.inj_text_restorers[nj_child.p_ft] then
      vim.notify(
        'No restorer defined for filetype: ' .. tostring(nj_child.p_ft),
        vim.log.levels.WARN
      )
    elseif not nj_child.p_text_meta then
      ...
      ---@type boolean, string[]?
      local restored_ok, restored_text = pcall(
        cfg.inj_text_restorers[nj_child.p_ft],
        rep_text,
        nj_child.p_text_meta,
        nj_child.p_indents
      )
  ...
  if not set_text_ok then
    ---@type string
    local err = 'ninjection.replace() error: Failed to set replacement text in parent buffer: '
      .. nj_child.p_bufnr
    if cfg.debug then
      vim.notify(err, vim.log.levels.ERROR)
    end
