...
M.reg_child_buf = function(p_bufnr, c_bufnr)
  ...
  if not svar_ok then
    ---@type string
    local err = 'buffer.reg_child_buf() error: Failed to set ninjection table var in parent bufnr: '
      .. p_bufnr
    if cfg.debug then
      vim.notify(err, vim.log.levels.ERROR)
    end
...
M.get_buf_child = function(c_bufnr)
  ...
  if not get_cnj_ok then
    ---@type string
    local err = 'ninjection.buffer.get_buf_parent() error: Error retrieving ninjection table for buffer '
      .. c_bufnr
    if cfg.debug then
      vim.notify(err, vim.log.levels.ERROR)
    end
  ...
  if not get_cnj_return or not NJChild.is_child(get_cnj_return) then
    ---@type string
    local err = 'ninjection.buffer.get_buf_parent() error: No child ninjection table for buffer: '
      .. c_bufnr
    if cfg.debug then
      vim.notify(err, vim.log.levels.ERROR)
    end
...
M.get_indents = function(bufnr)
  ...
  if not line_ok or not lines or #lines == 0 then
    ---@type string
    local err = 'ninjection.buffer.get_indents() error: Unable to retrieve lines from bufnr '
      .. bufnr
    if cfg.debug then
      vim.notify(err, vim.log.levels.ERROR)
    end
...
M.restore_indents = function(text, indents)
  ...
  elseif type(text) == 'table' then
    ...
    local err =
      'ninjection.buffer.restore_indents() error: Text must be a string or a table of lines.'
    ---@type string err
    if cfg.debug then
      vim.notify(err, vim.log.levels.ERROR)
    end
