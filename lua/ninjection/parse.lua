...
local function get_capture_pair(bufnr, cursor_pos, ft, root, query)
  ...
  ---@type string
  local lang_pattern = cfg.inj_lang_comment_pattern and cfg.inj_lang_comment_pattern[ft]
    or '#%s*([%w%p]+)%s*'
...
M.get_injection = function(bufnr)
  ...
  if not ft then
    ---@type string
    local err = 'ninjection.parse.get_injection() error: failed to retrieve filetype ... '
      .. tostring(ft_err)
    if cfg.debug then
      vim.notify(err, vim.log.levels.ERROR)
    end
  ...
  if not cursor_pos then
    ---@type string
    local err = 'ninjection.parse.get_injection() error: failed to get cursor position ... '
      .. tostring(cur_err)
    if cfg.debug then
      vim.notify(err, vim.log.levels.ERROR)
    end
  ...
  if not parsed_query then
    ---@type string
    local err = 'ninjection.parse.get_injection() warning: failed to get parsed query ... '
      .. tostring(qry_err)
    if cfg.debug then
      vim.notify(err, vim.log.levels.WARN)
    end
  ...
  if not root then
    ---@type string
    local err = 'ninjection.parse.get_injection() error: failed to get parse root node ... '
      .. tostring(root_err)
    if cfg.debug then
      vim.notify(err, vim.log.levels.ERROR)
    end
  ...
  if not capture then
    ---@type string
    local err = 'ninjection.parse.get_injection() warning, no injected language found: '
      .. tostring(cap_err)
    if cfg.debug then
      vim.notify(err, vim.log.levels.WARN)
    end
...
M.get_visual_range = function(node, bufnr)
  ...
  if not range or #range < 4 then
    if cfg.debug then
      vim.notify(
        'ninjection.parse.get_visual_range() warning: No valid '
          .. ' selection range found in range()',
        vim.log.levels.WARN
      )
    end
    return nil
  ...
  ---@type NJRange
  local vs_range =
    { s_row = range[1], s_col = visual_s_col, e_row = range[3], e_col = visual_e_col }
